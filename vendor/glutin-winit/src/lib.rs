...
fn create_display(
    ...
    let _preference = match _api_preference {
        ...
        ApiPreference::FallbackEgl => {
            // rsou patch: eframe 0.36 hard-codes `FallbackEgl`, but the
            // GLX implementation in UOS 20 / Mesa 18.3.6 can report an
            // asynchronous GLXBadContextTag while the first window is mapped.
            // Prefer EGL on X11 to avoid that broken GLX path, while retaining
            // GLX as a fallback for Linux installations where EGL is missing.
            DisplayApiPreference::EglThenGlx(Box::new(register_xlib_error_hook))
        }
    };
