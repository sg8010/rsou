...
- **已加入索引清单分两个页签**:「已添加文件夹」按来源文件夹树形展示(可展开看
  下属文档,文件夹本身可定位/重解析/移除),「单独文件」列出用「添加文件」直接
  加进来的文档。移除文件夹只删索引记录,不动磁盘上的文件
- **所有移除都需二次确认**:确认框说明影响范围,并明确告知不删磁盘文件
- **路径显示**:入库路径用平台惯用形式(Windows 下是 `C:\Users\…`,不带
  `\\?\` 前缀),检索结果与「目录前缀」过滤都按这一形式匹配;从旧版本升上来时
  会自动把存量路径归一化
- **检索语法**
...
| 文件夹树 | egui_ltreeview 0.9(与 egui 0.36 同版,零额外依赖,纯 Rust) |
...
├── normalize.rs   # 开库时的一次性路径归一化(剥 Windows \\?\ 前缀)
